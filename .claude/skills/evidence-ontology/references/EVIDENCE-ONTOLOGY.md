...
### [H] vs [A] Preference by Bucket

**Default to [H] in Ideal and Delta.** Uncertain claims about target state or transformation path are hypotheses until proven otherwise. Hypotheses demand falsification: the first test should try to disprove the claim, not confirm it.

**[A] is the fallback when falsification cost exceeds utility.** An assumption is a deliberate choice to move forward without testing. Because assumptions propagate risk to every downstream decision built on them, every [A] MUST include `Risk-if-wrong` documenting what breaks if the assumption fails.

| Bucket | Default uncertain tag | When to use the other |
|---|---|---|
| **BASELINE** | [OBS] or [M] — verify it. Minimize [A]. | Use [A] only when verification is genuinely blocked (no access, user must provide context). |
| **IDEAL** | [H] — testable claim about target state. Negate-first. | Use [A] only when cost of testing > utility. Must include Risk-if-wrong. |
| **DELTA** | [H] — testable claim about transformation path. Negate-first. | Use [A] only when cost of testing > utility. Must include Risk-if-wrong. |

**Why this matters:** [H] with negate-first strengthen creates a path to [F-LK] (confirmed) or revised [H] (refuted). [A] creates a path to risk propagation. The system prefers falsifiable claims over provisional ones because falsification resolves uncertainty; assumptions merely defer it.
