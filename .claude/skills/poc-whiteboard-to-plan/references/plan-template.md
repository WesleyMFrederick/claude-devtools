...
## Whiteboard Decision + Outcome Coverage

<!-- COVERAGE RULES:
  Every tagged item from the whiteboard must appear here.
  O-NNN outcomes are MANDATORY — each needs a success/acceptance criterion.
  A plan that cannot prove it delivers its outcomes has no verification path.
-->
